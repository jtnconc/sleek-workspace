...
interface WorkspaceState {
  ...
  /** Whether the notes call-history panel is open (controlled from Settings). */
  notesHistoryOpen: boolean;
  quote: QuoteDoc;
...
interface WorkspaceApi extends WorkspaceState {
  ...
  setWidgetTwoColumn: (id: string, twoColumn: boolean) => void;
  ...
  const api = useMemo<WorkspaceApi>(
    () => ({
      ...
      setNotesHistoryOpen: (open) =>
        setState((s) => ({ ...s, notesHistoryOpen: open })),
      saveNoteVersion: () =>
        setState((s) => ({
          ...s,
          noteHistory: [
      ...
      setWidgetTwoColumn: (id, twoColumn) =>
        patchWidgets((ws) => ws.map((x) => (x.id === id ? { ...x, twoColumn } : x))),
      clearPulse: (id) =>
        setState((s) => {
