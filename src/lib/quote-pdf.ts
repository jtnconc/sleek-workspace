...
const GRAY_COMPANY = hexToRgb("#5f5f5f") as [number, number, number];
...
export function buildQuotePdf(quote: QuoteDoc, hotel: HotelTemplate, logoImage?: string) {
  ...
  if (quote.company.trim()) {
    doc.setFont("helvetica", "bold").setFontSize(12).setTextColor(...GRAY_COMPANY);
    doc.text(quote.company.toUpperCase(), M, y, { charSpace: 0.4 });
    y += 17;
  }
  y += 10;
  ...
  if (emailLines.length || phoneLines.length) {
    ...
    emailLines.forEach((line) => {
      doc.setFont("helvetica", "bold").setTextColor(...headerFill(hotel.id, hotel.accent));
      doc.text(line, x, y);
