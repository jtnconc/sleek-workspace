...
const REMINDER_FILTERS: FilterChipOption[] = [
  { value: "flagged", label: "Flagged", icon: FlagPennant, accent: "red" },
  { value: "scheduled", label: "Scheduled", icon: CalendarDots, accent: "orange" },
  { value: "completed", label: "Completed", icon: CheckCircle, accent: "yellow" },
];

const TASK_FILTERS: FilterChipOption[] = [
  { value: "urgent", label: "Urgent", icon: Alarm, accent: "red" },
  { value: "scheduled", label: "Scheduled", icon: CalendarDots, accent: "orange" },
  { value: "completed", label: "Completed", icon: CheckCircle, accent: "green" },
];

const CONTACT_FILTERS: FilterChipOption[] = [
  { value: "hotel", label: "Hotel", icon: Building, accent: "blue" },
  { value: "agent", label: "Agents", icon: Users, accent: "purple" },
  { value: "client", label: "Clients", icon: User, accent: "neutral" },
      ...
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger asChild>
          <button
            ...
            onClick={stop}
            className={cn(
              "flex size-6 shrink-0 items-center justify-center rounded-full text-muted-foreground transition-opacity duration-200 hover:text-foreground hover:opacity-100 focus-visible:opacity-100 data-[state=open]:opacity-100",
              revealed ? "opacity-100" : "opacity-0 group-hover:opacity-40",
            )}
          >
            <DotsThreeCircle className="size-4" />
          </button>
        </PopoverTrigger>
        <PopoverContent
          side="bottom"
          ...
          >
            ...
            >
              {done ? (
                ...
                <Alarm className="size-2.5" weight="fill" style={{ color: accent }} />
              ) : null}
            </button>
              ...
              {isConfirming ? (
                ...
                <>
                  ...
                  >
                    <Alarm
                      className="size-3"
                      weight={t.priority === "urgent" ? "fill" : "regular"}
                      style={t.priority === "urgent" ? { color: "#1e3a5f" } : undefined}
                    />
                  </MiniAction>
          ...
          >
            ...
            >
              {done ? (
                ...
                <FlagPennant className="size-2.5" weight="fill" style={{ color: accent }} />
              ) : null}
            </button>
              ...
              {isConfirming ? (
                ...
                <>
                  ...
                  >
                    <FlagPennant
                      className="size-3"
                      weight={r.flagged ? "fill" : "regular"}
                      style={r.flagged ? { color: "#1e3a5f" } : undefined}
                    />
                  </MiniAction>
          ...
          <li
            ...
            style={{
              backgroundColor: n.pinned
                ? widget.tint
                  ? `color-mix(in oklab, ${tintVar(widget.tint)} 45%, white 55%)`
                  : "color-mix(in oklab, var(--primary) 10%, transparent)"
                : widget.tint
                  ? `color-mix(in oklab, ${tintVar(widget.tint)} 60%, white 40%)`
                  : undefined,
            }}
