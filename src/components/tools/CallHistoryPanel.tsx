...
import type { NoteRefItem } from "@/workspace/types";
import { parseEntities, toISODate } from "@/lib/note-parser";
import { sanitizeHtml } from "@/lib/sanitize-html";

/** Unified shape rendered by this panel, built from saved notes. */
...
export function CallHistoryPanel({ notes = [] }: NotesHistoryPanelProps) {
  ...
  const merged = useMemo<HistoryEntry[]>(() => {
    ...
    return plain.sort((a, b) => b.savedAtISO.localeCompare(a.savedAtISO));
  }, [notes]);
  ...
  const filtered = useMemo(() => {
    ...
    return merged.filter((e) => {
      ...
      return e.hashtags.some((h) => h.toLowerCase().includes(needle));
    });
  }, [merged, query]);
